...
var UserAppModel_pd = Backbone.Model.extend({  
	  ...
	  //APP STATE
	    ...
	    midPanelCollection: {"deviceName":"starterPanel", "deviceId":-1}, //"Active" Mid Panels associated with this device, viewed in center. Set by the Sidebar View -- activeDevice[0] is the deviceName, activeDevice[1] is the device's 'id'
	    rightPanelCollection: {"deviceName":"","deviceId":-1},  //"Former" or "Last Active" Right Panels associated with this device. 
	    leftPanelCollection: {"deviceName":"","deviceId":-1},  //"Shifted Left" Panels only displayed to left when user explicitly clicks on lastActiveDevice, prompting UI panels left.
	    ...
	    supportedUnitTypes: {'powerDraw':'watts','energyConsumption': "kWh"},
	    
	    panelFocus: 0, //The focus of a particular panel/graph (within a 'collection' of panels) will affect the position of all panels
	    
	    toClearPanels: {"deviceName":"","deviceId":-1}, // Panels check to see if they need to be cleared from DOM upon updating
	    ...
	    currentSidebar: {}, //referring to the sidebar view being rendered 
      ...
      //When the user clicks on a new device, this function is called to 1.) set the lastActiveDevice equal to activeDevice, 2.) setActiveDevice equal to new value, and 3.) trigger the 'device_select_event'
	...
	//IF the clicked device in UI is not currently represented by any of the panel collections, execute this action
	  if (selectedDevice_.deviceId != this.get('midPanelCollection').deviceId && selectedDevice_.deviceId != this.get('rightPanelCollection').deviceId && selectedDevice_.deviceId != this.get('leftPanelCollection').deviceId)
		...
		this.set({'toClearPanels': this.get('rightPanelCollection')}); //PanelView will remove panels stored as 'toClearPanels' in the userModel
		this.set({'rightPanelCollection':{}}); //clear right panel collection
	      }
	      if (this.get('leftPanelCollection').deviceName) {
		//console.log('left Panel to be cleared');
		this.set({'toClearPanels': this.get('leftPanelCollection')});  //this.set({'leftPanelCollection':{}});
		this.set({'leftPanelCollection':{}});
	      }
	      
	      this.set({'rightPanelCollection': this.get('midPanelCollection')}); // move the current mid panels to the right
	      this.set({'midPanelCollection': selectedDevice_}); // set new current mid panels
	  ...
	  if (selectedDevice_.deviceName == this.get('rightPanelCollection').deviceName)
		{
			...
			this.set({'leftPanelCollection': this.get('midPanelCollection')}); //the mid panels are moved to left collection
			this.set({'midPanelCollection': this.get('rightPanelCollection')}); //the right panels are moved to mid
			this.set({'rightPanelCollection': {}});
/*
...
*/
	...
	//IF the clicked device in UI is currently associated with the leftPanelCollection, execute this action
	  ...
	  {
			this.set({'rightPanelCollection': this.get('midPanelCollection')}); //the mid panels are moved to right
			this.set({'midPanelCollection': this.get('leftPanelCollection')}); //the left panels are moved to mid
			//this.set({'toClearPanels': this.get('leftPanelCollection')}); //remove the view for leftPanelCollection
			this.set({'leftPanelCollection': {}}); //reset the leftPanelCollection
	...
	//IF the clicked device in UI is currently associated with the midPanelCollection, execute this action
	  ...
	  //Sorry for the verboseness below, a simple this.save(this.defaults) wasn't firing a change event, so I stuck to the long way
	  //this.save({	midPanelCollection: {"deviceName":"starterPanel", "deviceId":-1}, rightPanelCollection: {"deviceName":"","deviceId":-1}, leftPanelCollection: {"deviceName":"","deviceId":-1}, supportedGraphs: ['powerDraw', 'energyConsumption','bombo'], panelFocus: 0, toClearPanels: {"deviceName":"","deviceId":-1}, token: "Token here", name: "Name here", householdId: "Household ID here", householdName: "Household name",expiration: 0, structure: {},id: 1, activeDevice: "starterPanel",lastActiveDevice: ''});  // same as: {token: "Token here",doneLoadingActions: false, name: "Name here", expiration: 0, householdId: "Household ID here", householdName: "Name here",  structure: {}}
